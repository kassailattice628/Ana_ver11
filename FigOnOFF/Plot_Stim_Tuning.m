...
switch s.pattern
    case 'Size_rand'
        %imgobj.dFF_s_ave(DataPoints, Sizes, ROIs)
        %stim_list = 1:5;
        
        %ON value
        on_mat = zeros(5, length(imgobj.selectROI));
        %OFF value
        off_mat = on_mat;
        ...
        for k = 1:length(imgobj.selectROI)
            ...
            off_mat(:,k) = max(imgobj.dFF_s_ave(p_off:p_off+p_duration, :, imgobj.selectROI(k)))';
            figure
            plot(stim_list, on_mat(:,k), 'o-g')
            hold on
            plot(stim_list, off_mat(:,k), 'o-r')
            hold off
    ...
    case {'Sin', 'Rect', 'Gabor', 'MoveBar'}
        ...
        for k = 1:imgobj.maxROIs
            ...
            imgobj.L_ori(k) = abs(sum(R_all_dir .* exp(2*1i*dir))/sum(R_all_dir));
